...
import './DashboardServiceMap.css';
...
interface DashboardServiceMapProps {
  alerts: AlertType[];
  nodes: Node[];
  edges: Edge[];
  loading?: boolean;
...
export const DashboardServiceMap: React.FC<DashboardServiceMapProps> = ({ 
  alerts, 
  nodes, 
  edges, 
  loading = false, 
  ...
  // Simpler dark theme detection - check if background is dark
  ...
  console.log('Theme debug:', { 
    ...
    isDarkTheme,
    hex: token.colorBgContainer ? parseInt(token.colorBgContainer.replace('#', ''), 16) : 'N/A'
  });
  ...
  const [layoutStyle, setLayoutStyle] = useState<'hierarchical' | 'static' | 'clustered'>('static');
  const [autoRefresh, setAutoRefresh] = useState<boolean>(false);
  const [refreshInterval, setRefreshInterval] = useState<number>(300); // 5 minutes in seconds

  // Custom tooltip state for styled tooltips
  ...
  const hasMinimalFilters = useCallback(() => {
    // Conservative check - warn if likely to return very large result set
    return nodes.length < 50; // If current graph already has 50+ nodes, full chain will be much larger
  }, [nodes.length]);
  ...
  // Stop animation function (defined early to avoid hook order issues)
  ...
  const processNodes = (): EnhancedNode[] => {
    return nodes
      ...
      .map(node => {
        ...
        const getNodeColor = () => {
          if (!isService) {
            return node.nodeType === 'namespace' ? '#8c8c8c' : '#1890ff'; // Grey for namespaces
          }
          
          const severityColors = {
            fatal: isDarkTheme ? '#8c8c8c' : '#000000', // Grey in dark mode, black in light mode
            critical: '#ff4d4f',
            warning: '#faad14',
            none: '#52c41a'
          };
        ...
        // Enhanced tooltip with rich HTML information
        const buildTooltip = () => {
          // Theme-aware colors
          ...
          if (node.external_calls && node.external_calls.length > 0) {
            const callsHtml = node.external_calls.slice(0, 3).map(call => 
              `<div style="font-size: 11px; color: ${tertiaryTextColor};">• <span style="color: #1890ff;">${call.method || 'GET'}</span> ${call.host}${call.path || ''} <span style="color: ${secondaryTextColor};">(${call.count}x)</span></div>`
            ).join('');
            
            sectionsHtml.push(`
              <div style="margin-bottom: 8px;">
                <div style="font-size: 12px; font-weight: 500; color: ${primaryTextColor}; margin-bottom: 4px;">External HTTP Calls:</div>
                ${callsHtml}
                ${node.external_calls.length > 3 ? `<div style="font-size: 11px; color: ${secondaryTextColor};">... and ${node.external_calls.length - 3} more</div>` : ''}
              </div>
            `);
          }

          // Database calls section
          if (node.database_calls && node.database_calls.length > 0) {
            const dbCallsHtml = node.database_calls.slice(0, 3).map(call => {
              const operation = call.operation ? `${call.operation} ` : '';
              const dbName = call.name ? ` (${call.name})` : '';
              return `<div style="font-size: 11px; color: ${tertiaryTextColor};">• ${operation}<span style="color: #722ed1;">${call.system}</span>${dbName} @ ${call.host || 'unknown'} <span style="color: ${secondaryTextColor};">(${call.count}x)</span></div>`;
            }).join('');
            
            sectionsHtml.push(`
              <div style="margin-bottom: 8px;">
                <div style="font-size: 12px; font-weight: 500; color: ${primaryTextColor}; margin-bottom: 4px;">Database Calls:</div>
                ${dbCallsHtml}
                ${node.database_calls.length > 3 ? `<div style="font-size: 11px; color: ${secondaryTextColor};">... and ${node.database_calls.length - 3} more</div>` : ''}
              </div>
            `);
          }

          // RPC calls section
          if (node.rpc_calls && node.rpc_calls.length > 0) {
            const rpcCallsHtml = node.rpc_calls.slice(0, 3).map(call => {
              const method = call.method ? `.${call.method}` : '';
              return `<div style="font-size: 11px; color: ${tertiaryTextColor};">• <span style="color: #13c2c2;">${call.service}</span>${method} <span style="color: ${secondaryTextColor};">(${call.count}x)</span></div>`;
            }).join('');
            
            sectionsHtml.push(`
              <div style="margin-bottom: 8px;">
                <div style="font-size: 12px; font-weight: 500; color: ${primaryTextColor}; margin-bottom: 4px;">RPC Calls:</div>
                ${rpcCallsHtml}
                ${node.rpc_calls.length > 3 ? `<div style="font-size: 11px; color: ${secondaryTextColor};">... and ${node.rpc_calls.length - 3} more</div>` : ''}
              </div>
            `);
          }
        ...
        // Add grey border for fatal nodes in dark theme for better visibility
        const getBorderConfig = () => {
          if (isService && severity === 'fatal' && isDarkTheme) {
            return {
              color: '#d9d9d9', // Light grey border for fatal nodes in dark theme
              width: 4 // Thicker border to ensure visibility
            };
          }
          return {
            color: undefined, // Use default border color
        ...
        return {
          ...
          font: { 
            ...
            color: isDarkTheme ? (isService ? '#a0a6b8' : '#ffffff') : '#ffffff', // White for namespaces in dark theme
            background: isService ? 'rgba(0, 0, 0, 0.8)' : 'transparent', // No background for namespaces
          ...
          // Enhanced shadow for alert nodes
          chosen: alertCount > 0 ? {
            node: (values: any) => {
          ...
          // Store tooltip content for custom tooltip handling
          tooltipContent: buildTooltip()
        };
      });
  };

  // Process edges with enhanced styling - differentiate service vs namespace dependencies
  const processEdges = (): EnhancedEdge[] => {
    return edges.map(edge => {
      ...
      // Classify edge types:
      // 1. Service-to-Service dependency: Blue, solid line
      // 2. Service-to-Namespace attachment: Grey, solid line  
      // 3. Namespace-to-Namespace dependency: Purple, dotted line
  ...
  // Get layout configuration based on selected style
    ...
    switch (layoutStyle) {
      case 'hierarchical':
        return {
          hierarchical: {
            enabled: true,
            direction: "TB", // Top to bottom for better visibility
            sortMethod: "directed",
  ...
  // Get physics configuration - use minimal physics for initial layout then disable
  const getPhysicsConfig = () => {
    if (layoutStyle === 'hierarchical') {
      return {
        enabled: false, // Hierarchical doesn't need physics
        stabilization: { enabled: false }
      };
    }
    
    return {
      enabled: true, // Enable briefly for initial positioning
      stabilization: {
        enabled: true,
        iterations: 100, // Quick stabilization
        updateInterval: 10
      },
      solver: 'repulsion',
      repulsion: {
  ...
  const renderEnhancedGraph = () => {
    ...
    try {
      ...
      const options = {
        nodes: {
          ...
          chosen: {
            node: (values: any, _id: any, selected: boolean, hovering: boolean) => {
              // Enhance selection and hover effects
        ...
        edges: {
          color: {
            color: isDarkTheme ? 'rgba(100, 200, 255, 0.8)' : '#2B7CE9', // Default - will be overridden by processEdges
            highlight: '#00d4aa',
            hover: '#00d4aa'
          },
          width: 2,
      ...
      // Handle tooltip position updates on mouse move
      network.on('dragStart', () => {
        setCustomTooltip(prev => ({ ...prev, visible: false }));
      });

      // Handle stabilization and disable physics after layout
      network.on('stabilizationIterationsDone', () => {
        ...
        // Start pulsing animation after stabilization
        setTimeout(() => {
  ...
  // Cleanup effect for component unmount and theme changes
  useEffect(() => {
    return () => {
  ...
  }, [isDarkTheme]); // Re-create network when theme changes
  ...
  const startPulsingAnimation = useCallback(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      const currentNodes = nodesDataSetRef.current.get(); // Get current nodes in dataset
      ...
      currentNodes.forEach(node => {
        ...
        if (isService && alertCount > 0) {
          // Calculate pulse rate based on severity
          ...
          if (pulseRate > 0) {
            ...
            const pulseIntensity = (Math.sin(phase * 2 * Math.PI) + 1) / 2; // 0 to 1
            
            const baseSize = focusMode ? 20 : 25;
            const pulsedSize = baseSize + (pulseIntensity * 5); // Add 0-5 pixels
            
            // Get proper border width based on node properties
            const baseBorderWidth = (isService && severity === 'fatal' && isDarkTheme) ? 4 :
                                  (isService && alertCount > 0) ? 3 : 2;
            const pulsedBorderWidth = baseBorderWidth + (pulseIntensity * 1); // Add 0-1 pixel pulse
      ...
      // Batch update all pulsing nodes
      if (updates.length > 0) {
  ...
  useEffect(() => {
    ...
    edges,
    alerts,
    isDarkTheme // Re-render when theme changes
  ]);
      ...
      <Space direction="vertical" size={16} style={{ width: '100%' }}>
        ...
        {totalServices === 0 ? (
          ...
          <div
            ...
            ref={graphRef}
            className={`enhanced-graph-container star-field-container ${focusMode ? 'focus-mode' : ''}`}
            data-dark-theme={isDarkTheme ? 'true' : 'false'}
            data-has-alerts={alerts.length > 0 ? 'true' : 'false'}
            style={{ 
              height: focusMode ? '320px' : '500px',
              position: 'relative',
